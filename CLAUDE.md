...
12. **Shell Environment Configuration** (`add_to_shell_config`):
    ...
    - **Generation guard**: the env file self-guards on `_FUMITM_ENV_GENERATION`, a 12-hex sha256 digest of the managed var/value set (`_env_generation()`). A shell that already carries the current generation skips the file. Startup stubs bypass the guard via `_FUMITM_ENV_FORCE=1` (set and unset around the source, never exported) — without the force, a login shell's `.zshenv` stub would record the generation, a vendor block in `.zprofile` would override a variable, and the `.zlogin` stub would no-op instead of winning by last-export-wins. The file also carries a `# fumitm-managed-vars:` manifest (union of everything ever managed there); on a generation change it unsets the manifest before exporting, so removed variables disappear from open shells.
    - **Refresh hook** (issue #100 Option B): a passive prompt hook in a separate `# >>> fumitm refresh (prompt hook) >>>` marker block — distinct markers so an older fumitm rewriting its managed block cannot erase it. zsh gets a `precmd_functions` entry in `.zshrc`, bash a `PROMPT_COMMAND` entry in `.bashrc` (interactivity-gated), fish an auto-loaded `~/.config/fish/conf.d/fumitm_refresh.fish`. The hook only sources the local env file (no subprocess, no network, no fumitm executable), preserves `$?`, and registers idempotently. It makes already-open shells pick up later fumitm runs — any copy, any invocation path, including Jamf `--run-as-user` runs — at the next prompt. Guarded sourcing means a user's same-generation manual export is not fought. Hook lifecycle is reconciled once per install run by `_reconcile_refresh_hook()` in `main()`, independent of tool outcomes — a fully converged system (every tool `already_ok`) still gains the hook on upgrade, and `--no-refresh-hook` removes an already-installed hook (only the refresh block / fish conf.d file; stubs stay) via `_remove_refresh_hooks()`. The step reports as the `refresh-hook` entry in the run summary, runs behind `_run_hook_reconciliation()`'s exception-to-`ToolResult` boundary (a hook I/O failure becomes a failed `refresh-hook` entry, never a summary-bypassing crash), and is excluded from the succeeded tally in `_print_summary()` via `_AUXILIARY_TOOLS` — a hook install cannot soften an all-tools-failed exit 1 into a 3, though its failure still marks the run partial and its install still drives `changes_made`. The first-ever run still needs the chained source command or a new shell. Env files are written atomically (temp + `os.replace` in `_write_managed_file(atomic=True)`) so a concurrent prompt never sources a partial file; startup files keep in-place writes because they may be user symlinks into dotfile repos.
    - **fish** now mirrors the POSIX design with its own generated `~/.config/fumitm/env.fish` (`set -gx` syntax, same generation digest as env.sh so cross-shell children skip re-export; the generation-change cleanup uses `set -eg` — global scope only — because an unscoped `set -e` erases from the smallest existing scope and would permanently delete a user's `set -Ux` universal value from fish's persistent store): `config.fish` gets a stub, a legacy inline block is hoisted into env.fish (`_add_to_fish_config`). csh derivatives keep the historical inline block via `_write_inline_block()`.
    ...
    - `_uses_env_file(shell_type)` gates the POSIX env-file path; fish branches to `_add_to_fish_config` before that check, and csh derivatives fall through to the inline block.

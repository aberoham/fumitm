...
4. **Tool-Specific Setup Functions**:
   ...
   - Gradle uses a fumitm-managed PKCS12 snapshot when Aikido is absent. When Aikido is active, setup first ensures the active JDK contains the available proxy roots, then removes only fumitm's matching `custom-cacerts` override outside vendor blocks. The Aikido-detected state survives root-extraction failure, and Aikido/JDK trust stays in control without losing the provider root.

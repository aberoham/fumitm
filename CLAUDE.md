...
3. **Certificate Management**:
   ...
   - **Aikido supplemental root** (`SUPPLEMENTAL_ROOTS` dict): auto-detected and added to every managed bundle alongside the primary provider root (`--with-aikido`/`--no-aikido`/`--aikido-cert`). The `aikido-adopt` registry tool is the forward path (macOS-only — Aikido's adoption record lives under `/Library/Application Support`, so elsewhere the step is skipped): on agents shipping the `aikido-doctor` CLI it runs `[sudo] aikido-doctor certconfig adopt <staged-root>`, with the doctor resolved to an absolute path through a root-ownership PATH check (sudo secure_path may not carry the user's PATH). That check rejects any component owned by a non-root user or writable by others, and rejects group-writable components unless the group is in `PRIVILEGED_GROUPS` (wheel, admin) — macOS ships `/Applications` as `root:admin drwxrwxr-x`, and rejecting that outright made every agent installed as an application bundle permanently undiscoverable, so the whole adoption step silently skipped on every stock Mac. Membership of `admin` already confers sudo, so honoring it surrenders no boundary, while `root:staff` (staff contains every local user) stays rejected. The provider root is the provider root staged into a private mkstemp copy (`_stage_adoption_cert`) so the user-writable cert file cannot be swapped between fumitm's checks and the privileged read, so Aikido's own CA bundles trust the primary provider. `_aikido_trusts_root` treats the **bundles** as ground truth: the root must appear in **every** bundle Aikido builds (`_aikido_bundles_contain`, globbing `run_dir` for `*-combined-ca.pem` and `*-cafile.pem`), because those files are what the tools Aikido configures actually read. They must be checked as a set because they disagree — on an observed host the pip and node bundles carried the Netskope chain (130 certs, seeded from the System keychain) while the openssl and ruby bundles did not (128, seeded from Apple's static `/etc/ssl/cert.pem`) — so the original single-bundle check reported success and skipped the adoption that would have fixed the other two. Aikido's `adopted-cas/<sha256>.pem` record (`_aikido_has_adopted`) is deliberately **not** accepted in place of that: a root adopted once still vanishes from any bundle Aikido later rebuilds, and a stale record would assert trust that no longer exists. The record answers only for an agent exposing no bundles at all. Re-adopting converges rather than looping, verified against agent 1.7.28: one `certconfig adopt` reinstalls every rule and took the openssl and ruby bundles from 128 to 130 certificates while creating the record directory, which Aikido writes lazily on first adoption (filenames are the full SHA-256, though the CLI displays a truncated form). `endpoint-protection-proxy-ca-crt.pem` is deliberately outside the globs — it holds Aikido's own root alone and could never satisfy the check. The env-var reclaim, curlrc-override fix, and stub-last shell machinery remain as defense-in-depth for hosts where adopt hasn't run.
